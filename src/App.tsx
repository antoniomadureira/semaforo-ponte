...
export default function App() {
  ...
  // Alteração para um proxy alternativo mais rápido (Liza.io ou similar)
  ...
  const verificarPonte = async () => {
    try {
      // Se já temos dados, não mostramos "A ligar" para evitar piscar o ecrã
      if (!inicializadoRef.current) setMensagem('A consultar APDL...');

      const response = await fetch(`${API_URL}${API_URL.includes('?') ? '&' : '?'}t=${Date.now()}`, { 
        cache: 'no-store',
        signal: AbortSignal.timeout(10000) // Dá 10 segundos ao proxy antes de desistir
      });
      ...
      // Só mostra erro se não tivermos dados nenhuns guardados
  ...
  useEffect(() => {
    verificarPonte();
    // Aumentado para 15 segundos para não saturar o proxy gratuito
