...
export default function App() {
  ...
  // PROXY ALTERNATIVO: CodeTabs (Geralmente mais rápido e menos bloqueado)
  const API_URL = isProd 
    ? 'https://api.codetabs.com/v1/proxy?url=' + encodeURIComponent('https://siga.apdl.pt/AberturaPonteMovel/')
    : '/api-apdl';
  ...
  const verificarPonte = async () => {
    try {
      setMensagem('A LIGAR AO PROXY...');
      
      const controller = new AbortController();
      const id = setTimeout(() => controller.abort(), 15000); // Timeout manual de 15s

      const response = await fetch(`${API_URL}${API_URL.includes('?') ? '&' : '?'}cache=${Date.now()}`, { 
        cache: 'no-store',
        signal: controller.signal
      });
      
      clearTimeout(id);
      setMensagem('A LER DADOS DA PONTE...');
      ...
      if (!inicializadoRef.current) {
        setMensagem('REDE LENTA - A REPETIR...');
        setCor('OFF');
      }
    }
  };
  ...
  useEffect(() => {
    verificarPonte();
    const apiTimer = setInterval(verificarPonte, 20000); // Aumentado para 20s para estabilidade
